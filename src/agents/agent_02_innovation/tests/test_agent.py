"""
Tests for Agent 2: Innovation Scorer
...
from src.agents.agent_02_innovation.agent import score_innovation
...
async def test_single_technology():
    ...
    print("\n" + "="*80)
    print("TEST 1: Single Technology Innovation Scoring")
    print("="*80)
    ...
    try:
        ...
        print(f"\n[RESULT] Innovation Score: {output.innovation_score:.1f}/100")
        print(f"[RESULT] Confidence: {output.confidence}")
        print(f"\n[REASONING] {output.reasoning}")

        # Print key metrics
        metrics = output.key_metrics
        print(f"\n[METRICS] Patent Activity:")
        print(f"  - Patent count (2yr): {metrics.patent_count_2yr}")
        print(f"  - Patent citations: {metrics.patent_citations}")
        print(f"  - PageRank-weighted count: {metrics.patent_pagerank_weighted:.1f}")
        print(f"  - Avg PageRank: {metrics.avg_patent_pagerank:.6f}")

        print(f"\n[METRICS] Paper Activity:")
        print(f"  - Paper count (2yr): {metrics.paper_count_2yr}")
        print(f"  - Paper citations: {metrics.paper_citations}")

        print(f"\n[METRICS] Community Context:")
        print(f"  - Community ID: {metrics.community_id}")
        print(f"  - Community patents: {metrics.community_patent_count}")
        print(f"  - Community papers: {metrics.community_paper_count}")

        # Temporal trend
        print(f"\n[METRICS] Temporal Trend: {metrics.temporal_trend}")

        # Validation
        assert 0 <= output.innovation_score <= 100, "Score must be 0-100"
...
async def test_three_technologies():
    ...
    try:
        ...
        for tech in tech_discovery.technologies:
            ...
            print(f"{tech.id:<30} {tech.domain or 'Unknown':<20} {output.innovation_score:<10.1f} {output.confidence:<12}")

        # Validation
        assert len(results) == 3, "Should score 3 technologies"
        for output in results:
            assert 0 <= output.innovation_score <= 100, f"{output.tech_id}: Score out of range"
...
async def test_batch_scoring():
    ...
    try:
        ...
        for i, tech in enumerate(tech_discovery.technologies, start=1):
            ...
            results.append(output)
            print(f"Score: {output.innovation_score:.1f}")

        # Statistics
        scores = [r.innovation_score for r in results]
        ...
        sorted_results = sorted(results, key=lambda x: x.innovation_score, reverse=True)

        print(f"\n[STATS] Top 5 Innovation Scores:")
        print(f"{'Rank':<6} {'Tech ID':<30} {'Score':<10}")
        print("-" * 50)
        for i, r in enumerate(sorted_results[:5], start=1):
            print(f"{i:<6} {r.tech_id:<30} {r.innovation_score:<10.1f}")

        print(f"\n[STATS] Bottom 5 Innovation Scores:")
        print(f"{'Rank':<6} {'Tech ID':<30} {'Score':<10}")
        print("-" * 50)
...
async def main():
    ...
    print("\n" + "="*80)
    print("AGENT 2: INNOVATION SCORER - TEST SUITE")
    print("="*80)
