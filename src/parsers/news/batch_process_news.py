"""
Batch Processor for News Articles
...
from parsers.news.news_parser import NewsParser, load_industry_config
from parsers.news.news_extractors import load_articles_from_file
...
def prepare_inputs(
    ...
    Returns:
        List of input dicts with index, article_data
    """
    inputs = []

    end_index = min(len(articles), start_index + limit) if limit else len(articles)
...
def process_single_article(
    ...
    try:
        print(f"\n[{index}] Processing: {title[:60]}...")

        result = parser.parse_article(article_data)
...
def process_batch_concurrent(
    ...
    """
    Process articles concurrently with ThreadPoolExecutor.
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_input):
            ...
            try:
                ...
                error_record = {
                    "index": idx,
                    "url": inp["article_data"].get("url", "Unknown"),
                    "title": inp["article_data"].get("title", "Unknown"),
                    "error": str(e)
                }
...
def save_checkpoint(
    ...
    Args:
        results: List of parsed articles
...
def merge_checkpoints(
    ...
    print(f"  Total articles: {len(deduplicated)} (after deduplication)")
    ...
    all_output_path = os.path.join(
        output_dir,
        f"all_news_scored_{range_str}_{timestamp}.json"
    )
    ...
    print(f"  Relevant articles (quality >= {quality_threshold}): {len(relevant)}")

    relevant_output_path = os.path.join(
        os.path.dirname(output_dir),
        "relevant_evtol_news.json"
    )
...
def main():
    parser_args = argparse.ArgumentParser(description="Batch process news articles")

    parser_args.add_argument("--config", default="configs/evtol_config.json", help="Industry config file")
    parser_args.add_argument("--input", default="data/eVTOL/news_sentiment/articles.json", help="Input JSON file with articles")
    parser_args.add_argument("--output", default="data/eVTOL/news_sentiment", help="Output directory")
    parser_args.add_argument("--start", type=int, default=0, help="Starting index")
    parser_args.add_argument("--limit", type=int, default=None, help="Max articles to process (None = all)")
    ...
    parser_args.add_argument("--checkpoint", type=int, default=50, help="Save checkpoint every N articles")
    parser_args.add_argument("--no-resume", action="store_true", help="Disable resume from checkpoint")

    args = parser_args.parse_args()

    print("="*80)
    print("NEWS ARTICLES BATCH PROCESSING")
    print("="*80)
    ...
    # [3/6] Load news articles
    print(f"\n[3/6] Loading news articles: {args.input}")
    articles = load_articles_from_file(args.input)
    print(f"  Total articles found: {len(articles)}")

    # [4/6] Prepare inputs
    print(f"\n[4/6] Preparing inputs...")
    inputs = prepare_inputs(articles, start_index=args.start, limit=args.limit)
    print(f"  Prepared {len(inputs)} article inputs")

    # Setup checkpoint manager
    checkpoint_dir = os.path.join(args.output, "batch_processing", "checkpoints")
    checkpoint_file = os.path.join(checkpoint_dir, ".checkpoint_news_batch.json")

    checkpoint_mgr = ScholarlyCheckpointManager(checkpoint_file)

    resume = not args.no_resume
    if resume and checkpoint_mgr.get_completed_count() > 0:
        print(f"  Resume mode: Skipping {checkpoint_mgr.get_completed_count()} already-processed articles")
    ...
    print(f"  Remaining to process: {len(inputs)} articles")

    print(f"\n[CHECKPOINT] Checkpoint system active")
    print(f"  Checkpoint interval: Every {args.checkpoint} articles")
    ...
    if inputs:
        ...
        print("\n[DONE] All articles already processed!")
        return
    ...
    print(f"\n  Output files:")
    print(f"    All scores: {args.output}/batch_processing/all_news_scored_{range_str}_{timestamp}.json")
    print(f"    Relevant: {args.output}/relevant_evtol_news.json")
    print(f"    Errors: {args.output}/batch_processing/processing_errors_{range_str}_{timestamp}.json")

    # [5/6] Process articles in batches
    print(f"\n[5/6] Processing articles...")
    print("-"*80)
    ...
    for batch_start in range(0, len(inputs), args.checkpoint):
        ...
        print(f"\n[BATCH] Processing articles {batch_start}-{batch_end-1}")
