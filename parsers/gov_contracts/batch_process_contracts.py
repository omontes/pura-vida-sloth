"""
Batch Processor for Government Contracts
Author: Pura Vida Sloth Intelligence System

Processes USASpending.gov contract data with:
...
from parsers.gov_contracts.gov_contracts_parser import GovContractsParser, load_industry_config
from parsers.gov_contracts.contract_extractors import load_contracts_from_file
...
def prepare_inputs(
    ...
    Returns:
        List of input dicts with index, contract_data
    """
    inputs = []

    end_index = min(len(contracts), start_index + limit) if limit else len(contracts)

    for idx in range(start_index, end_index):
        inputs.append({
            "index": idx,
            "contract_data": contracts[idx]
        })
...
def process_single_contract(
    parser: GovContractsParser,
    contract_data: Dict[str, Any],
    index: int
) -> Dict[str, Any]:
    """
    Process a single government contract.

    Args:
        parser: GovContractsParser instance
    ...
    Returns:
        Parsed result dict
    """
    award_id = contract_data.get('Award ID', 'Unknown')
    recipient = contract_data.get('Recipient Name', 'Unknown')

    try:
        print(f"\n[{index}] Processing: {award_id} - {recipient}")

        result = parser.parse_contract(contract_data)
...
def process_batch_concurrent(
    ...
    """
    Process contracts concurrently with ThreadPoolExecutor.
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_input):
            ...
            try:
                ...
                error_record = {
                    "index": idx,
                    "award_id": inp["contract_data"].get("Award ID", "Unknown"),
                    "recipient": inp["contract_data"].get("Recipient Name", "Unknown"),
                    "error": str(e)
                }
                errors.append(error_record)
                print(f"  [ERROR] Contract {idx}: {e}")
...
def save_checkpoint(
    ...
    Args:
        results: List of parsed contracts
...
def merge_checkpoints(
    ...
    print(f"  Total contracts: {len(deduplicated)} (after deduplication)")
    ...
    all_output_path = os.path.join(
        output_dir,
        f"all_gov_contracts_scored_{range_str}_{timestamp}.json"
    )
    ...
    print(f"  Relevant contracts (quality >= {quality_threshold}): {len(relevant)}")

    relevant_output_path = os.path.join(
        os.path.dirname(output_dir),
        "relevant_evtol_gov_contracts.json"
    )
...
def main():
    parser_args = argparse.ArgumentParser(description="Batch process government contracts")

    parser_args.add_argument("--config", default="configs/evtol_config.json", help="Industry config file")
    parser_args.add_argument("--input", default="data/eVTOL/government_contracts/contracts.json", help="Input JSON file with contracts")
    parser_args.add_argument("--output", default="data/eVTOL/government_contracts", help="Output directory")
    parser_args.add_argument("--start", type=int, default=0, help="Starting index")
    parser_args.add_argument("--limit", type=int, default=None, help="Max contracts to process (None = all)")
    ...
    parser_args.add_argument("--checkpoint", type=int, default=100, help="Save checkpoint every N contracts")
    parser_args.add_argument("--no-resume", action="store_true", help="Disable resume from checkpoint")

    args = parser_args.parse_args()

    print("="*80)
    print("GOVERNMENT CONTRACTS BATCH PROCESSING")
    print("="*80)
    ...
    print(f"\n[2/6] Initializing GovContractsParser")
    ...
    # [3/6] Load government contracts
    print(f"\n[3/6] Loading government contracts: {args.input}")
    contracts = load_contracts_from_file(args.input)
    print(f"  Total contracts found: {len(contracts)}")

    # [4/6] Prepare inputs
    print(f"\n[4/6] Preparing inputs...")
    inputs = prepare_inputs(contracts, start_index=args.start, limit=args.limit)
    print(f"  Prepared {len(inputs)} contract inputs")

    # Setup checkpoint manager
    checkpoint_dir = os.path.join(args.output, "batch_processing", "checkpoints")
    checkpoint_file = os.path.join(checkpoint_dir, ".checkpoint_contracts_batch.json")

    checkpoint_mgr = ScholarlyCheckpointManager(checkpoint_file)

    resume = not args.no_resume
    if resume and checkpoint_mgr.get_completed_count() > 0:
        print(f"  Resume mode: Skipping {checkpoint_mgr.get_completed_count()} already-processed contracts")
    ...
    print(f"  Remaining to process: {len(inputs)} contracts")

    print(f"\n[CHECKPOINT] Checkpoint system active")
    print(f"  Checkpoint interval: Every {args.checkpoint} contracts")
    ...
    if inputs:
        ...
        print("\n[DONE] All contracts already processed!")
        return
    ...
    print(f"\n  Output files:")
    print(f"    All scores: {args.output}/batch_processing/all_gov_contracts_scored_{range_str}_{timestamp}.json")
    print(f"    Relevant: {args.output}/relevant_evtol_gov_contracts.json")
    print(f"    Errors: {args.output}/batch_processing/processing_errors_{range_str}_{timestamp}.json")

    # [5/6] Process contracts in batches
    print(f"\n[5/6] Processing contracts...")
    print("-"*80)
    ...
    for batch_start in range(0, len(inputs), args.checkpoint):
        ...
        print(f"\n[BATCH] Processing contracts {batch_start}-{batch_end-1}")
